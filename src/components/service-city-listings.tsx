...
/**
 * Contractor listings for one service category in one fixed city (from the URL),
 * independent of the global city picker.
 */
export function ServiceCityListings({
  serviceKey,
  citySlug,
  cityName,
}: {
  ...
  const contractors = useQuery(
    ...
    getConvexEnv().configured ? { serviceKey, citySlug } : 'skip',
  );
  ...
  if (list.length === 0) {
    return (
      <div className="rounded-xl border border-navy/10 bg-navy/5 p-8 text-center">
        <p className="text-lg font-semibold text-navy">{t('emptyInCity', { city: cityName })}</p>
